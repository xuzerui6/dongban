...
export function PhoneShell({ children, route, go, hideNav = false }: { children: React.ReactNode; route: Route; go: (r: Route) => void; hideNav?: boolean }) {
  ...
  return <div className="stage">
    ...
    <div className="phone">
      ...
      {!hideNav && <nav className="bottom-nav">{tabs.map(t => <button key={t.route} className={route === t.route ? 'active' : ''} onClick={() => go(t.route)}><Icon name={t.icon} size={21}/><span>{t.label}{t.route === 'avatar' && state.newlyUnlockedOutfits.length > 0 && <em className="nav-new">NEW</em>}</span></button>)}</nav>}
